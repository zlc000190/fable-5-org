...
async function getCanonicalUrl(canonicalUrl: string, locale: string) {
  ...
  if (canonicalUrl.startsWith('http')) {
    ...
    // Strip trailing slash for non-default locale only (per locale SEO convention),
    // then ALWAYS re-add trailing slash so canonical is normalized to https://hailuo2.video/path/
