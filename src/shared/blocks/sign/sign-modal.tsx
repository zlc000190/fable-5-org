...
export function SignModal({ callbackUrl = '/' }: { callbackUrl?: string }) {
  ...
  if (isDesktop) {
    return (
      <Dialog open={isShowSignModal} onOpenChange={handleOpenChange}>
        <DialogContent className="sm:max-w-[425px] dark bg-black border-white/10">
          <DialogHeader>
  ...
  return (
    <Drawer open={isShowSignModal} onOpenChange={handleOpenChange}>
      <DrawerContent className="dark bg-black border-white/10">
        ...
        <DrawerFooter className="pt-4">
          <DrawerClose asChild>
            <Button variant="outline" className="rounded-full">{t('cancel_title')}</Button>
          </DrawerClose>
        </DrawerFooter>
      </DrawerContent>
    </Drawer>
