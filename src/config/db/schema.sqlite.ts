...
export const affiliate = table('affiliate', {
  ...
  paidOrderNo: text('paid_order_no').notNull().default(''),
  paidAmount: integer('paid_amount').notNull().default(0),
  rewardPercent: integer('reward_percent').notNull().default(0),
...
export const video = table(
  'video',
  {
    ...
    prompt: text('prompt').notNull(),
    model: text('model').notNull(),
    parameters: text('parameters').notNull(), // JSON string storing generation parameters
    status: text('status').notNull().default('generating'), // generating, completed, failed, uploading
    ...
    fileSize: integer('file_size'), // Video file size in bytes
    duration: text('duration'), // Video duration (e.g., "6s")
    resolution: text('resolution'), // Video resolution (e.g., "768p")
    replicatePredictionId: text('replicate_prediction_id'), // Replicate prediction ID for tracking
    ...
    updatedAt: integer('updated_at', { mode: 'timestamp_ms' })
      ...
      .$onUpdate(() => /* @__PURE__ */ new Date())
      .notNull(),
    completedAt: integer('completed_at', { mode: 'timestamp_ms' }), // When generation completed
  },
  (table) => [
