...
export const affiliate = table('affiliate', {
  id: varchar191('id').primaryKey(),
  userId: varchar191('user_id')
    .notNull()
    .references(() => user.id, { onDelete: 'cascade' }),
  status: varchar('status', { length: 50 }).notNull().default(''),
  invitedBy: varchar191('invited_by').notNull(),
  paidOrderNo: varchar191('paid_order_no').notNull().default(''),
  paidAmount: int('paid_amount').notNull().default(0),
  rewardPercent: int('reward_percent').notNull().default(0),
...
export const video = table(
  'video',
  {
    ...
    prompt: longtext('prompt').notNull(),
    model: varchar191('model').notNull(),
    parameters: longtext('parameters').notNull(), // JSON string storing generation parameters
    status: varchar('status', { length: 50 }).notNull().default('generating'), // generating, completed, failed, uploading
    originalVideoUrl: text('original_video_url'), // Original URL from Replicate
    videoUrl: text('video_url'), // R2 stored video URL
    startImageUrl: text('start_image_url'), // Start image URL for image-to-video generation
    firstFrameImageUrl: text('first_frame_image_url'), // First frame thumbnail URL
    fileSize: int('file_size'), // Video file size in bytes
    duration: varchar('duration', { length: 50 }), // Video duration (e.g., "6s")
    resolution: varchar('resolution', { length: 50 }), // Video resolution (e.g., "768p")
    replicatePredictionId: varchar191('replicate_prediction_id'), // Replicate prediction ID for tracking
  ...
  (table) => [
    // Query user's videos by status
    index('idx_video_user_status').on(table.userId, table.status),
    // Query videos by replicate prediction ID
    index('idx_video_prediction_id').on(table.replicatePredictionId),
  ]
);
